...
export default {
  ...
  //  if (!this.$store.state.boolusuario){
    ...
    this.$api.get('permisos').then(res=>{
  ...
  methods: {
    ...
    onSubmit() {
      this.$q.loading.show();
         for(let i=0;i<11;i++){
               if(this.dato.apartment ==this.departamentos[i]){
                this.departamento_id=this.departamentos_ids[i]
               }
         }
      ...
      this.$api.post( "/users", {
        ...
        type:this.dato.type,
        apartment:this.dato.apartment,
        cod_apartment:this.departamento_id
      }).then((res) => {
        this.$q.notify({
          color: "green-4",
          textColor: "white",
    ...
    onMod() {
      this.$q.loading.show();
       for(let i=0;i<11;i++){
               if(this.dato2.apartment==this.departamentos[i]){
                this.departamento_id=(this.departamentos_ids[i])
               // console.log(this.departamentos_ids[i]);
               }
         }
      this.$api.put("/users/" + this.dato2.id, {name:this.dato2.name,
        password:this.dato2.password,
        email:this.dato2.email,
        fechalimite:this.dato2.fechalimite,
        type:this.dato2.type,
        apartment:this.dato2.apartment,
        cod_apartment:this.departamento_id
      }).then((res) => {
          this.$q.notify({
            color: "green-4",
    ...
    onDel() {
      this.$q.loading.show();
      this.$api.delete("/users/" + this.dato2.id)
        .then((res) => {
        this.$q.notify({
          color: "green-4",
          textColor: "white",
    ...
    cambiopass (item) {
     ...
     this.filter=item.row.email
      this.$q.dialog({
        ...
        this.$q.loading.show()
        this.$api.put('/pass/'+item.row.id,{password:data}).then(res=>{
        //  console.log(res.data);
